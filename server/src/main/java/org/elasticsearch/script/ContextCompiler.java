...
public class ContextCompiler<FactoryType> {

    private static final Logger logger = LogManager.getLogger(ScriptService.class);

    private final ScriptContext<FactoryType> context;
    ...
    public ContextCompiler(ScriptContext<FactoryType> context, int cacheMaxSize, TimeValue cacheExpire,
        ...
        logger.debug("context [{}]: using script cache with max_size [{}], expire [{}]", this.context.name, cacheMaxSize, cacheExpire);
    ...
    public FactoryType compile(ScriptEngine scriptEngine, String id, String idOrCode, ScriptType type, Map<String, String> options) {
            ...
            if (compiledScript == null) {
                try {
                    ...
                    if (logger.isTraceEnabled()) {
                        logger.trace("context [{}]: compiling script, type: [{}], lang: [{}], options: [{}]", this.context.name, type,
                            lang, options);
                    }
    ...
    void checkCompilationLimit() {
        ...
        if (scriptsPerTimeWindow >= 1) {
            ...
            throw new CircuitBreakingException("[script] Too many dynamic script compilations within, max: [" +
                ...
                "this limit can be changed by the [script.context." + context.name + ".max_compilations_rate] setting",
    ...
    private class ScriptCacheRemovalListener implements RemovalListener<CacheKey, Object> {
        ...
        public void onRemoval(RemovalNotification<CacheKey, Object> notification) {
            if (logger.isDebugEnabled()) {
                logger.debug("context [{}]: removed [{}] from cache, reason: [{}]", context.name, notification.getValue(),
